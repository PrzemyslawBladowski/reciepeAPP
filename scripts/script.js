...
function szukajPrzepisow() {
	...
	recipeContainer.innerHTML =
		'<p class="placeholder-text">Szukam pyszności...</p>'
	...
	fetch(API_URL + haslo)
		...
		.catch(error => {
			...
			recipeContainer.innerHTML =
				'<p class="placeholder-text">Coś poszło nie tak. Spróbuj ponownie.</p>'
		})
}
...
function wyswietlPrzepisy(przepisy) {
	...
	if (przepisy === null) {
		recipeContainer.innerHTML =
			'<p class="placeholder-text">Nie znaleziono przepisów dla tego hasła. Spróbuj "Chicken", "Pie", "Soup".</p>'
		return
	}
	...
	przepisy.forEach(przepis => {
		...
		karta.innerHTML = `
            ...
            <div class="card-content">
                <span class="category">🍽️ ${przepis.strCategory} | 🌍 ${
			przepis.strArea
		}</span>
                <h3>${przepis.strMeal}</h3>
                <a href="${
									przepis.strSource || przepis.strYoutube
								}" target="_blank" class="btn-link">Zobacz przepis</a>
            </div>
        `
